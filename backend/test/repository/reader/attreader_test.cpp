...
GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(AttParamTest);
...
TEST_P(AttParamTest, ReadAttInstance) {
    ...
    EXPECT_EQ(problem->getEdgeWeightType(), EdgeWeightType::ATT);
...
TEST_P(AttParamTest, MatrixIsValidAdjacency) {
    const auto& matrix = problem->getGraph().getMatrix();
    int n = matrix.size();
