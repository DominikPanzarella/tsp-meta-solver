...
ConfigProvider::ConfigProvider() : m_cs{std::make_unique<ComputeStartingNode>()}, algoConfigure{false}
{
}
...
void ConfigProvider::configureAlgorithms()
{
...
void ConfigProvider::configureProblems(std::vector<std::shared_ptr<IProblem>> problems)
{

    if (!algoConfigure)
        throw std::runtime_error("Algorithms need to be configure first!");

    // SINGLE INSTANCE SETTINGS (NI)
    std::mt19937 rng_ni(m_ni->getSeed());
    ...
    for (const auto &problem : problems)
    {
        ...
        int startingNode = dist(rng_ni);

        auto instanceSetting = std::make_shared<NearestInsertionInstanceSetting>(startingNode);

        m_ni->addInstance(problem->getName(), instanceSetting);
    }

    // SINGLE INSTANCE SETTINGS (NN)
    std::mt19937 rng_nn(m_nn->getSeed());
    ...
    for (const auto &problem : problems)
    {
        ...
        std::uniform_int_distribution<int> dist(0, n - 1);
        int startingNode = dist(rng_nn);

        auto instanceSetting = std::make_shared<NearestNeighbourInstanceSetting>(startingNode);

        m_nn->addInstance(problem->getName(), instanceSetting);
    }


    // SINGLE INSTANCE SETTINGS
    std::mt19937 rng_fi(m_fi->getSeed());
...
void ConfigProvider::configureNI()
{
    ...
    if (niNode->contains("GeneralSettings"))
    {
        ...
        {
            seed = (*generalSetting)["SEED"]->get<int>();
        }
        ...
        if (generalSetting->contains("HardInstances"))
        {
            ...
            {
                auto nodeArray = (*hardInstances)["Nodes"];
                std::vector<int> nodes;

                if (nodeArray && nodeArray->is_array())
                {
                    const auto &arr = nodeArray->impl_->data;

                    for (const auto &node : arr)
                    {
                        nodes.push_back(node.get<int>());
                    }
                }
                else
                {
                    throw std::runtime_error("Hard instances dimension must be passed as an array");
                }
            ...
            {
                throw std::runtime_error("Nodes section missing in NearestInsertion settings");
            }
        }
        else
        {
            throw std::runtime_error("HardInstances section missing in NearestInsertion settings");
        }
    }
